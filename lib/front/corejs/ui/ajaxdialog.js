...
(function() {
  ...
  AjaxDialog.DIALOG_LOADING_CLASS = 'ajax-loading';

  // markup to use inside YUI Panel body while content is loading, 'style' will be set with width
  AjaxDialog.DIALOG_LOADING_HTML = '<div class="body '+AjaxDialog.DIALOG_LOADING_CLASS+'" style></div>';
  ...
  AjaxDialog.prototype = {
    ...
    init: function(srcMarkup, options)
    {
      ...
      if (options.requestUri)
      {
        ...
        var sStyle = options.mobile ? '' : ' style="width:' + options.width + 'px"';
        this.yPanel.setBody(AjaxDialog.DIALOG_LOADING_HTML.replace(/style/, sStyle));
      }
      ...
      this.yPanel.hideEvent.subscribe(function(){ that.onHideEvent(); }, this, true);
      // YUI Module destroy
      ...
      this.eventDel.on("JSDialogSuccess", function(){ that.handleDialogStatus(AjaxDialog.STATUS_SUCCESS); return false; });
      this.eventDel.on("JSDialogFail",    function(){ that.handleDialogStatus(AjaxDialog.STATUS_FAILED); return false; });
      this.eventDel.on("JSDialogClose",   function(){ that.destroy(); return false; });
      this.eventDel.on("JSDialogHide",    function(){ that.hide(); return false; });
    ...
    show: function()
    {
      ...
      if (this.options.requestUri && !this.ajaxPanel)
      {
        ...
        this.ajaxPanel.get(this.options.requestData || null, this.options.requestUri);
      }
