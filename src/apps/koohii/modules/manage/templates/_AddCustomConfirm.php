<?php use_helper('CJK', 'Form', 'Validation', 'Widgets'); ?>

<?= form_tag('manage/addCustomProcess', ['class' => 'main-form']); ?>

  <?php if ($countNewCards == 0 && $countExistCards): ?>

  <p> All <?= _CJ('kanji'); ?> in the selection are already present in your flashcards.</p>

  <?php else: ?>

  <p> <strong><?= $countNewCards; ?></strong> new card(s) will be added<?php if ($countExistCards) {
    echo sprintf(
      ' (%d are already in your flashcards)',
      $countExistCards
    );
  } ?>:</p>  

<?php include_partial('CharacterSelection', ['cards' => $newCards]); ?>

  <?php endif; ?>

  <p>
    <?php if ($countNewCards > 0) {
      echo _bs_submit_tag('Add Cards').'&nbsp;&nbsp;';
    } ?><a href="#" class="ko-Btn is-ghost JSManageCancel">Go back</a>
  </p>


</form>

