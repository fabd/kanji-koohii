...
?>
  ...
  <h2><?= escape_once($profile_user->username); ?><span>'s public profile</span></h2>

  <h3>Activity</h3>

  <table class="blocky mb-8">
    <tr><th style="width: 170px">Stories</th>
        <td><strong><?= $num_stories->total; ?></strong> (<?= $num_stories->public; ?> public,
        <?= $num_stories->private; ?> private)</td>
    </tr>
    <tr><th>Kanji Flashcards</th><td><strong><?= ReviewsPeer::getFlashcardCount($profile_uid).'</strong> ('.ReviewsPeer::getReviewedFlashcardCount($profile_uid).' reviewed)';
?></td></tr>
    <tr><th>Total Reviews</th><td><?= ReviewsPeer::getTotalReviews($profile_uid); ?></td></tr>
    <tr><th>Joined</th><td><?= date('j M Y', $profile_user->ts_joindate); ?></td></tr>
    <tr><th>Last Login</th><td><?= time_ago_in_words($profile_user->ts_lastlogin, true); ?> ago</td></tr>
  </table>
...
<?php elseif ($sf_user->isAuthenticated()): ?>
  ...
  <p><strong><?= $num_stories->public; ?></strong> stories have been shared by <?= escape_once($profile_user->username); ?>.
