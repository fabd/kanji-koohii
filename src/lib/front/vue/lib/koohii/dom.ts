 ...
 * BROWSER SUPPORT
 *
 *   Recent versions of Edge, Chrome, Safari, iOS Safari, Chrome for Android.
 ...
 *    css({ prop1: value, ...})
 *
 *   each(callback)                    ... iterate over returned elements
 *
...
class DomJS<EL extends Element> implements DomJSInterface<EL>, ArrayLike<EL> {
  // ArrayLike
  length: number;
  [n: number]: EL;
